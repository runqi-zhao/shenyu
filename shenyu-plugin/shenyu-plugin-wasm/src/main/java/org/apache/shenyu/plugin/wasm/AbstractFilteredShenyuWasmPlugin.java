...
public class AbstractFilteredShenyuWasmPlugin extends AbstractWasmPlugin {
    ...
    protected  Mono<Void> doExecute(ServerWebExchange exchange, ShenyuPluginChain chain, SelectorData selector, RuleData rule) {
        ...
        final WasmRuleHandle wasmRuleHandle = WasmPluginDataHandler.CACHED_HANDLE.get().obtainHandle(CacheKeyUtils.INST.getKey(rule));
        //set wam handle param
    ...
    private SelectorData obtainSelectorDataCacheIfEnabled(final String path) {
        Object[] paths = this.instance.exports.getFunction("handle").apply(path);
        if (Objects.isNull(paths)) {
            return null;
        }
        String pathCacheKey = (String) paths[0];
        return selectorMatchConfig.getCache().getEnabled() ? MatchDataCache.getInstance().obtainSelectorData(named(), pathCacheKey) : null;
    }
    ...
    private RuleData obtainRuleDataCacheIfEnabled(final String path) {
        ...
        String pathCacheKey = (String) paths[0];
        return ruleMatchConfig.getCache().getEnabled() ? MatchDataCache.getInstance().obtainRuleData(named(), pathCacheKey) : null;
    }
